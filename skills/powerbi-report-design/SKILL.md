...
description: >-
  ...
  `powerbi://skills/powerbi-report-authoring`. For end-to-end requirements, approval, and build
  sequencing, use `powerbi://skills/powerbi-report-planning`. Triggers: "design Power BI report",
...
**Scope boundary** — This skill decides *what* a report should look like and *why*. It does **not** call MCP tools. After producing a `Design Brief:`, hand off to the MCP report-authoring tools (`create_empty_report`, `add_page`, `add_visual`, `add_field_to_visual`, etc.) for implementation.
...
- Hand off all implementation to MCP tools after producing the `Design Brief:`; this skill does not call any tool directly.
...
Before routing archetypes or picking charts, commit to a **tone** and **signature**. The tone captures the report's feel; the signature is the one defining visual move that recurs across pages. If the prompt lacks a specific identity, use `powerbi://skills/powerbi-report-design/references/tone-catalog` and `powerbi://skills/powerbi-report-design/references/signatures` to choose, remix, or author a tone/signature. Do not force-fit catalog entries; the requirement is a concrete identity that drives palette, typography, layout, and recurring visual treatment. For brownfield redesigns, read `powerbi://skills/powerbi-report-design/references/brownfield` and capture both current and target tone/signature.
...
|--------|-----------|-----------|
| C-suite, board, GM; ≤10 s scan; "is it on track?" | **Executive Summary** | `powerbi://skills/powerbi-report-design/references/archetypes/executive-summary` |
| Shift operator, NOC, wallboard; "is it broken?" | **Operational Monitor** | `powerbi://skills/powerbi-report-design/references/archetypes/operational-monitor` |
| Analyst; hypothesis testing; "why did X happen?" | **Analytical Canvas** | `powerbi://skills/powerbi-report-design/references/archetypes/analytical-canvas` |
| Author-driven argument; "here's what happened" | **Narrative Story** | `powerbi://skills/powerbi-report-design/references/archetypes/narrative-story` |
| Ranking, benchmarking, variance; "relative to what?" | **Comparative Benchmark** | `powerbi://skills/powerbi-report-design/references/archetypes/comparative-benchmark` |
...
For multi-page reports, see [`powerbi://skills/powerbi-report-design/references/archetype-composition`](powerbi://skills/powerbi-report-design/references/archetype-composition) — common compositions (Executive + Drill, Ops + Detail, Story + Evidence, Multi-domain) + cross-page variant rotation rules. Avoid mono-archetype reports; same-archetype pages must rotate variants where data signals support it.

### Step 3 — Chart Selection

Read [`powerbi://skills/powerbi-report-design/references/chart-selection`](powerbi://skills/powerbi-report-design/references/chart-selection) to match each analytical question to the right visual type. Respect the encoding hierarchy: position → length → angle → area → hue. Sample the data first — a line chart with flat lines or a bar chart with two bars indicates the wrong visual choice.

### Step 4 — Visual Configuration

Read `powerbi://skills/powerbi-report-design/references/visual-cookbook` for per-visual-type rules: sort order, color strategy, label placement, axis config, and conditional formatting. These rules determine whether charts look professional or amateur. Read `powerbi://skills/powerbi-report-design/references/layout` before placing visuals on the canvas.

### Step 5 — Theme

Adapt the report theme to the design identity from Step 1. For generated reports,
start from `powerbi://skills/powerbi-report-design/assets/base` or preserve its critical per-type safeguards when
...
version when adapting `powerbi://skills/powerbi-report-design/assets/base` — use the `powerbi://skills/powerbi-report-authoring`
skill.

### Step 6 — Canonical Design Contract

Emit a structured `Design Brief:` YAML block. This is the contract with
`powerbi://skills/powerbi-report-authoring`. Populate every field with concrete values from Steps
...
Keep this minimal shape in mind; use `powerbi://skills/powerbi-report-design/references/design-brief` for the full
...
and the one decision. See `powerbi://skills/powerbi-report-design/references/design-brief` for examples.

### Step 7 — Review and Handoff

Before handing off, read `powerbi://skills/powerbi-report-design/references/design-brief` and
`powerbi://skills/powerbi-report-design/references/pre-flight-checklist`, then critically review the design
contract against both. For brownfield, also walk the brownfield-specific items
in `powerbi://skills/powerbi-report-design/references/brownfield`. Fix problems before handing off.
...
Once the contract passes the checklist, hand off to the MCP authoring tools for implementation.
